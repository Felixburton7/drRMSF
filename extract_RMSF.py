...
# Alternatively, if grouping fails or is undesirable, we can parse
# 'pdbProteinAtoms' to reconstruct (resid, resname) in PDB order,
# matching the domain's 'numResidues' attribute. 

# No 'UNK' placeholders are used: we strictly produce valid residue IDs
# and names for each of the 141 (or however many) residues.
...
#   4) Extract 'rmsf' (must be length == numResidues).
#   5) Save CSV in a temperature_<temp> folder.
#   6) Compute an average RMSF across all valid combos, saving to 'average_RMSF'.
...
#     """
#     Command-line arguments:
#       --h5_file   : Path to the mdCATH .h5 file (e.g. 'mdcath_dataset_1r9lA02.h5')
#       --domain    : Domain identifier (e.g. '1r9lA02')
#       --output_dir: Output folder for CSVs
#       --verbose   : If set, prints logs and warnings
#     """
#     parser = argparse.ArgumentParser(
#         description="Extract RMSF data from an mdCATH .h5 file, ensuring correct residue names and indices."
#     )
...
#         "--h5_file",
#         required=True,
#         help="Path to the mdCATH .h5 file."
#     )
#     parser.add_argument(
#         "--domain",
#         required=True,
#         help="Domain identifier in the .h5 (e.g. '1r9lA02')."
#     )
#     parser.add_argument(
#         "--output_dir",
#         default="rmsf_output",
#         help="Directory for storing RMSF CSV files (default: 'rmsf_output')."
#     )
#     parser.add_argument(
#         "--verbose",
#         action="store_true",
...
#     for line in lines:
#         # Typical ATOM/HETATM line format:
#         # Col 17-20 => resname, col 22-26 => resid
#         if line.startswith("ATOM") or line.startswith("HETATM"):
#             resname = line[17:20].strip()
#             try:
#                 resid = int(line[22:26].strip())
...
#     The logic:
#       1) If domain_group['resid'].shape == numResidues => Great, directly use them.
#       2) Else if 'pdbProteinAtoms' is present => parse PDB lines for unique residue info
#          of length numResidues.
#       3) Else attempt grouping from the 2165-atom arrays. 
#       4) If all fails, raise an error.
...
#     create_directory(args.output_dir)
#     avg_dir = os.path.join(args.output_dir, "average_RMSF")
#     create_directory(avg_dir)

#     # Will store each valid RMSF array for averaging
#     all_rmsf = []
...
#     with h5py.File(args.h5_file, "r") as h5f:
#         if args.domain not in h5f:
#             raise KeyError(f"Domain '{args.domain}' not found in {args.h5_file}")

#         dom_group = h5f[args.domain]
#         num_residues = dom_group.attrs["numResidues"]

#         # Attempt to build (resid, resname) of length == num_residues
#         resid_data, resname_data = determine_resid_resname(dom_group, verbose=args.verbose)
#         if len(resid_data) != num_residues or len(resname_data) != num_residues:
#             raise ValueError("[ERROR] Final mismatch: resid/resname does not match numResidues after determination.")

#         # Identify temperature groups (digits only)
#         temps = sorted([k for k in dom_group.keys() if k.isdigit()])
#         if len(temps) == 0 and args.verbose:
#             print(f"[WARNING] No numeric temperature groups found in '{args.domain}'.")

#         combos = []
#         for t in temps:
#             reps = sorted(dom_group[t].keys())
#             for r in reps:
#                 combos.append((t, r))

#         if args.verbose:
#             print(f"[INFO] Found {len(combos)} (temperature, replica) combos.")

#         pbar = tqdm(combos, desc="Extracting RMSF", dynamic_ncols=True, bar_format="{l_bar}{bar}| {percentage:5.1f}% [{elapsed}<{remaining}]")
#         for (temp, rep) in pbar:
#             rep_group = dom_group[temp][rep]
#             if "rmsf" not in rep_group:
#                 if args.verbose:
#                     print(f"[WARNING] 'rmsf' missing at {args.domain}/{temp}/{rep}. Skipping.")
#                 continue
#             data = rep_group["rmsf"][:]
#             if data.shape[0] != num_residues:
#                 if args.verbose:
#                     print(f"[WARNING] 'rmsf' shape {data.shape} != {num_residues}. Skipping {temp}/{rep}.")
#                 continue
...
#             # Save to temperature_<temp> folder
#             t_folder = os.path.join(args.output_dir, f"temperature_{temp}")
#             create_directory(t_folder)

#             out_name = f"{args.domain}_{temp}_{rep}_rmsf.csv"
#             df.to_csv(os.path.join(t_folder, out_name), index=False)

#             if args.verbose:
#                 print(f"[INFO] Saved {out_name}")
#             all_rmsf.append(data)

#     # Compute average if we have valid arrays
#     if len(all_rmsf) > 0:
#         stacked = np.vstack(all_rmsf)  # shape=(nCombos, num_residues)
...
#             "average_rmsf": avg
#         })
#         out_file = f"{args.domain}_average_rmsf.csv"
#         df_avg.to_csv(os.path.join(avg_dir, out_file), index=False)

#         if args.verbose:
#             print(f"[INFO] Saved average RMSF -> {out_file}")
#     else:
#         if args.verbose:
#             print("[WARNING] No valid RMSF found => no average file.")
